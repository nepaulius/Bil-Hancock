...
tikr+=genRandom4((KO[k]+golas)*int(hashas.back()));
    }
...
std::string ComputeMerkleRoot(std::vector<std::string> hashes) {
    ...
    {
        hashes[i] = kardinalus_pokyciai_2(hashes[2*i] + hashes[2*i + 1]);
    ...
    if (hashes.size() == 0) return nullptr;
    return hashes[0];
}
